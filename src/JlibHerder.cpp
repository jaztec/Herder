...
itsCHAR( tC )
{
	...
	if( ( box == 0 ) || ( clipsStanding == 0 ) || ( clipsWalkingLeft == 0 ) || ( clipsWalkingRight == 0 ) ||
			( clipsWalkingUp == 0 ) || ( clipsWalkingDown == 0 ) )
	{	
...
void BaseClass::play_sound( int index )
{
	...
	switch( index )
	{
		case( 1 ): if( sound1 ) Mix_PlayChannel( -1, sound1, 0 ); break;
		case( 2 ): if( sound2 ) Mix_PlayChannel( -1, sound2, 0 ); break;
		case( 3 ): if( sound3 ) Mix_PlayChannel( -1, sound3, 0 ); break;
	}
}
...
void Character::handle_events( SDL_Event& event )
{
	if( event.type == SDL_KEYDOWN )
	{
		...
		{
			case( SDLK_w ): yVel -= Y_SPEED; break;
			case( SDLK_s ): yVel += Y_SPEED; break;
			case( SDLK_a ): xVel -= X_SPEED; break;
			case( SDLK_d ): xVel += X_SPEED; break;
			default: break;
		}
	...
	if( event.type == SDL_KEYUP )
	{
		switch( event.key.keysym.sym )
		{
			case( SDLK_w ): yVel += Y_SPEED; break;
			case( SDLK_s ): yVel -= Y_SPEED; break;
			case( SDLK_a ): xVel += Y_SPEED; break;
			case( SDLK_d ): xVel -= X_SPEED; break;
			default: break;
		}
		handledMovement = true;
...
void Character::calc_state()
{
	...
	else if( xSpeed == 0 && ySpeed == 0 )
	{
		switch( lastState )
		{
			case( WALK_UP ): state = FACE_UP; break;
			case( WALK_DOWN ): state = FACE_DOWN; break;
			case( WALK_LEFT ): state = FACE_LEFT; break;
			case( WALK_RIGHT ): state = FACE_RIGHT; break;
...
void Character::show( SDL_Surface* screen )
{
	...
	switch( state )	
	{
		case( FACE_DOWN ): clip = &clipsStanding[ 0 ]; break;
		case( FACE_UP ): clip = &clipsStanding[ 1 ]; break;
		case( FACE_RIGHT ): clip = &clipsStanding[ 2 ]; break;
		case( FACE_LEFT ):	clip = &clipsStanding[ 3 ]; break;
		case( WALK_DOWN ): clip = &clipsWalkingDown[ actionFrame ]; break;
		case( WALK_UP ): clip = &clipsWalkingUp[ actionFrame ]; break;
		case( WALK_LEFT ): clip = &clipsWalkingLeft[ actionFrame ]; break;
		case( WALK_RIGHT ): clip = &clipsWalkingRight[ actionFrame ]; break;
		default: clip = &clipsStanding[ 0 ]; break;
	}
...
void Dog::show( SDL_Surface* screen )
{
	...
	switch( state )
	{
		case( FACE_DOWN ): clip = &clipsStanding[ 0 ]; break;
		case( FACE_UP ): clip = &clipsStanding[ 1 ]; break;
		case( FACE_LEFT ): clip = &clipsStanding[ 2 ]; break;
		case( FACE_RIGHT ):	clip = &clipsStanding[ 3 ]; break;
		case( WALK_DOWN ): clip = &clipsWalkingDown[ actionFrame ]; break;
		case( WALK_UP ): clip = &clipsWalkingUp[ actionFrame ]; break;
		case( WALK_LEFT ): clip = &clipsWalkingLeft[ actionFrame ]; break;
		case( WALK_RIGHT ): clip = &clipsWalkingRight[ actionFrame ]; break;
...
void Dog::set_following( int stance )
{
	...
	switch( stance )
	{
		case( 1 ): stopped = false; followCharacter = true; followRoute = false; break;
		case( 2 ): stopped = false; followCharacter = false; followRoute = true; break;
		case( 3 ): set_stopped(); break;
	}
}
...
/*bool Sheep::check_values()			****************Debug functie**********************
{
...
void SheepManager::do_think()
{
	...
	for( int i = 0; i < qtySheep; i++ )
	{
		...
		//else
			//itsSheep[ i ].random_movement( this, false );

	}

	// Onderstaand is de oude functie voor het berekenen van het weglopen voor poppetjes
	// Deze is momenteel omgeruild voor een eerste check via de nieuwe set_velocity()
	/*
	for( int i = 0; i < qtySheep; i++ )
	{
		if( !( itsSheep[ i ].is_finished() ) )
		{
			...
			check_closure( &( itsSheep[ i ] ), CHARACTER );
			check_closure( &( itsSheep[ i ] ), DOG );

			if( !( itsSheep[ i ].handledMovement ) )
				itsSheep[ i ].random_movement( true );
			else
				itsSheep[ i ].random_movement( false );
		}
	}
	*/
		...
		for( int i = 0; i < qtySheep; i++ )
		{
			if( itsSheep[ i ].get_its_number() != sheep->get_its_number() )
			{
				if( ( distance( sheep->get_box().x, sheep->get_box().y, itsSheep[ i ].get_box().x, itsSheep[ i ].get_box().y ) < max_distance )
					)//&& ( distance( sheep->get_box().x, sheep->get_box().y, itsSheep[ i ].get_box().x, itsSheep[ i ].get_box().y ) > 50 ) )
				{
...
void SheepManager::handle_collision( Sheep* sheep, COLLIDEDIRECTION collision )
{
	...
	{
		case( COLLISION_TOP ): y_vel += h; sheep->set_state( WALK_DOWN ); break;
		case( COLLISION_BOTTOM ): y_vel -= h; sheep->set_state( WALK_UP ); break;
		case( COLLISION_LEFT ): x_vel += w; sheep->set_state( WALK_RIGHT ); break;
		case( COLLISION_RIGHT ): x_vel -= w; sheep->set_state( WALK_LEFT ); break;
		case( COLLISION_TOPRIGHT ): y_vel += h; x_vel -= w; sheep->set_state( WALK_DOWN ); break; 
		case( COLLISION_TOPLEFT ): y_vel += h; x_vel += w; sheep->set_state( WALK_DOWN ); break; 
		case( COLLISION_BOTTOMRIGHT ): y_vel -= h; x_vel -= w; sheep->set_state( WALK_UP ); break; 
		case( COLLISION_BOTTOMLEFT ): y_vel -= h; x_vel += w; sheep->set_state( WALK_UP ); break; 
		default: break;
	}
...
void SheepManager::move( BaseClass* character, BaseClass* dog )
{
	...
	for( int i = 0; i < qtySheep; i++ )
	{
		if( !itsSheep[ i ].is_finished() )
		{
			...
			/*for( int j = 0; j < qtySheep; j++)
			{
				if( ( itsSheep[ j ].get_its_number() != itsSheep[ i ].get_its_number() ) && ( itsSheep[ j ].is_finished() == false ) && !( itsSheep[ j ].handledMovement ) )
				{
					set_velocity( &itsSheep[ j ], itsSheep[ i ].get_box( 1 ), itsSheep[ i ].is_scarsed() );
				}
			}*/
		}
	}

	// Onderstaande is de oude functie voor het bewegen van de schapen zonder de nieuwe functie set_velocity()
	// Voor nu uitgezet.
	/*
	for( int i = 0; i < qtySheep; i++ )
	{
		if( itsSheep[ i ].is_finished() == false )
		{
			itsSheep[ i ].move( *finish );
			COLLIDEDIRECTION collision;
			for( int j = 0; j < qtySheep; j++)
			{
				if( ( itsSheep[ j ].get_its_number() != itsSheep[ i ].get_its_number() ) && ( itsSheep[ j ].is_finished() == false ) && !( itsSheep[ j ].handledMovement ) )
				{
					collision.collision = check_collision( &( itsSheep[ i ] ), &( itsSheep[ j ] ) );
					if( collision.collision )
					{
						handle_collision( &( itsSheep[ i ] ), collision );
						itsSheep[ i ].handledMovement = true;
						itsSheep[ i ].move( *finish );
					}
				}
			}

			collision.collision = check_collision( &( itsSheep[ i ] ), dog );
			if( collision.collision )
			{
				handle_collision( &( itsSheep[ i ] ), collision );
				itsSheep[ i ].handledMovement = true;
			...
			collision.collision = check_collision( &( itsSheep[ i ] ), character ); 
			if( collision.collision )
			{
				handle_collision( &( itsSheep[ i ] ), collision );
				itsSheep[ i ].handledMovement = true;
