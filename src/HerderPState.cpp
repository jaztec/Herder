...
void HPlayState::handle_events( JEngine* game )
{
	...
	while( SDL_PollEvent( &event ) )
	{
		...
		switch( event.type )
		{
			...
			case( SDL_VIDEORESIZE ): game->toggle_fullscreen(); break;
			case( SDL_KEYDOWN ):
			{
				...
				if( event.key.keysym.sym == SDLK_SPACE )
                                {
                                    switch(cameraAim) {
                                        case(1): cameraAim = 2; break;
                                        case(2): cameraAim = 1; break;
                                        default: cameraAim = 1; break;
                                    }
                                }
...
void HPlayState::update( JEngine* game )
{
	...
	switch( cameraAim )
	{
		case( 1 ): character->set_camera( &map->camera ); break;
		case( 2 ): dog->set_camera( &map->camera ); break;
		default: character->set_camera( &map->camera ); break;
	}
}
